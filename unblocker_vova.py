...
# --- CONFIGURATION ---
# UTF-8 Encoding is CRITICAL for Vietnamese/Unicode filenames
...
class AutoUnblockerApp:
    def __init__(self, root):
        ...
        # [CHANGE 1] Added "Developed by Dr. Vova" to the title bar
    ...
    def _build_ui(self):
        ...
        # 2. Monitor Section (The "Automatic" Feature)
    ...
    def log_msg(self, msg, type="INFO"):
        """Thread-safe logging to GUI and File"""
    ...
    def toggle_monitoring(self):
        if not self.is_monitoring:
            ...
            self.lbl_mon_status.config(text="Status: Active - Scanning for blocked files...", foreground="green")
    ...
    def monitor_loop(self, folder_path):
        """Checks the folder every 3 seconds for blocked files."""
            ...
            try:
                for root, _, files in os.walk(folder_path):
                    ...
                    for name in files:
                        ...
                        self.check_and_fix(full_path, manual=False) # Manual=False prevents popups during auto-scan
                time.sleep(3)
    ...
    def check_and_fix(self, path, manual=False):
        """
        Checks for Zone.Identifier and removes it.
        [CHANGE 2] Added Messageboxes for user feedback when manual=True
        """
        ...
        if os.path.exists(zone_id):
            try:
                ...
                if manual:
                    messagebox.showinfo("Success", f"Process Finished Successfully!\n\nFile '{filename}' has been unblocked.\nYou can now open it.")
                    
            except PermissionError:
                # [CHANGE] Failure Message (Locked)
                msg = f"LOCKED: Could not fix '{filename}'.\nPlease CLOSE the file in Word/Excel first."
                self.log_msg(msg, "ERROR")
                if manual:
                    messagebox.showerror("Process Failed", f"The file is locked by another program.\n\nPlease close '{filename}' and try again.")
            
            except Exception as e:
                # [CHANGE] Failure Message (General)
                self.log_msg(f"Failed to fix '{filename}': {e}", "ERROR")
                if manual:
                    messagebox.showerror("Process Failed", f"An unexpected error occurred:\n{str(e)}")
        
        elif manual:
             # [CHANGE] Inform user file was already fine
             self.log_msg(f"File is already safe: {filename}", "INFO")
             messagebox.showinfo("Info", f"Process Finished.\n\n'{filename}' was already unblocked and safe.")

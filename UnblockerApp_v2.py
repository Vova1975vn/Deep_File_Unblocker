...
class FileUnblockerApp:
    def __init__(self, app_root):
        self.root = app_root
        self.root.title("Deep File Unblocker & Permission Fixer - ver 2.5 (Admin)")
    ...
    def _build_ui(self):
        ...
        self.btn_start = ttk.Button(frame_ctrl, text="START", command=self.start_thread, width=8)
        self.btn_start.pack(side="left", padx=5)
        
        self.btn_pause = ttk.Button(frame_ctrl, text="PAUSE", command=self.toggle_pause, state="disabled", width=8)
        self.btn_pause.pack(side="left", padx=5)
        
        self.btn_cancel = ttk.Button(frame_ctrl, text="CANCEL", command=self.cancel_process, state="disabled", width=8)
        self.btn_cancel.pack(side="left", padx=5)
        
        # Right-side buttons (Pack from right to left)
        self.btn_exit = ttk.Button(frame_ctrl, text="EXIT", command=self.root.destroy, width=8)
        self.btn_exit.pack(side="right", padx=5)

        self.btn_log = ttk.Button(frame_ctrl, text="MASTER", command=self.open_log_file, width=8)
        self.btn_log.pack(side="right", padx=5)

        self.btn_save_log = ttk.Button(frame_ctrl, text="SAVE", command=self.save_log_ui, width=8)
        self.btn_save_log.pack(side="right", padx=5)

        self.btn_copy_log = ttk.Button(frame_ctrl, text="COPY", command=self.copy_log_ui, width=8)
        self.btn_copy_log.pack(side="right", padx=5)

        self.btn_clear_log = ttk.Button(frame_ctrl, text="CLEAR", command=self.clear_log_ui, width=8)
    ...
    def open_log_file(self):
        """Opens the master log file in the default text editor."""
        if os.path.exists(LOG_FILENAME):
            os.startfile(LOG_FILENAME)
        else:
            messagebox.showinfo("Log Not Found", "No master log file has been generated yet.")

    def save_log_ui(self):
        """Prompts the user to save the current GUI log to a file."""
        log_content = self.txt_log.get("1.0", tk.END).strip()
        
        if not log_content:
            messagebox.showinfo("Empty", "There is no log data to save.")
            return

        file_path = filedialog.asksaveasfilename(
            defaultextension=".txt",
            filetypes=[("Text Files", "*.txt"), ("Log Files", "*.log"), ("All Files", "*.*")],
            title="Save Current Log As...",
            initialfile=f"manual_export_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
        )
    ...
    def copy_log_ui(self):
        """Copies the current GUI log text to the Windows clipboard."""
        log_content = self.txt_log.get("1.0", tk.END).strip()
        
        if not log_content:
            messagebox.showinfo("Empty", "There is no log data to copy.")
            return

        # Clear the clipboard and append the new text
        self.root.clipboard_clear()
        self.root.clipboard_append(log_content)
        self.root.update() # Required in Tkinter to finalize the clipboard action
        
        messagebox.showinfo("Copied", "Log successfully copied to clipboard!")

    def clear_log_ui(self):
        """Clears the visual Tkinter log window."""
        self.txt_log.config(state="normal")
        self.txt_log.delete(1.0, tk.END)
        self.txt_log.config(state="disabled")
        self.log_msg("--- GUI LOG CLEARED ---")
    ...
    def perform_file_ops(self, path):
        try:
            ...
            if not self.dry_run_active:
                stream_path = f"{win_path}:Zone.Identifier"
                subprocess.run(
                    ["cmd", "/c", "del", "/q", "/a", stream_path], 
            ...
            if not self.dry_run_active:
                ...
                result_takeown = subprocess.run(takeown_cmd, capture_output=True, text=True, errors="replace", creationflags=hide_window)
                
                if result_takeown.returncode != 0:
                    err_msg = result_takeown.stderr.strip()
                    if "insecure file systems" in err_msg or "ACL" in err_msg:
                        pass # Silently ignore FAT32 drives
                    else:
                        has_error = True
                        self.log_msg(f"Takeown failed on {os.path.basename(win_path)}: {err_msg}", level="ERROR")
                else:
                    icacls_cmd = ["icacls", win_path, "/grant", "Administrators:F", "/C", "/Q"]
                    result_icacls = subprocess.run(icacls_cmd, capture_output=True, text=True, errors="replace", creationflags=hide_window)
                    if result_icacls.returncode != 0:
                        has_error = True
                        self.log_msg(f"Icacls failed on {os.path.basename(win_path)}: {result_icacls.stderr.strip()}", level="ERROR")
...
if __name__ == "__main__":
    if is_admin():
        ...
        app = FileUnblockerApp(main_root)
        main_root.mainloop()
    else:

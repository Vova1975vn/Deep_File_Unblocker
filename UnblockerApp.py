...
# --- LOGGING SETUP ---
# This creates the background master log file
...
class FileUnblockerApp:
    def __init__(self, root):
        ...
        self.root.geometry("850x600") # Made slightly wider to fit new buttons
    ...
    def _build_ui(self):
        ...
        # Left-side buttons
        self.btn_start = ttk.Button(frame_ctrl, text="START", command=self.start_thread, width=10)
        self.btn_start.pack(side="left", padx=5)
        
        self.btn_pause = ttk.Button(frame_ctrl, text="PAUSE", command=self.toggle_pause, state="disabled", width=10)
        self.btn_pause.pack(side="left", padx=5)
        
        self.btn_cancel = ttk.Button(frame_ctrl, text="CANCEL", command=self.cancel_process, state="disabled", width=10)
        self.btn_cancel.pack(side="left", padx=5)
        
        # Right-side buttons (Pack from right to left)
        self.btn_exit = ttk.Button(frame_ctrl, text="EXIT", command=self.root.destroy, width=10)
        self.btn_exit.pack(side="right", padx=5)

        self.btn_log = ttk.Button(frame_ctrl, text="OPEN MASTER", command=self.open_log_file, width=14)
        self.btn_log.pack(side="right", padx=5)

        self.btn_save_log = ttk.Button(frame_ctrl, text="SAVE LOG", command=self.save_log_ui, width=10)
        self.btn_save_log.pack(side="right", padx=5)

        self.btn_clear_log = ttk.Button(frame_ctrl, text="CLEAR LOG", command=self.clear_log_ui, width=10)
    ...
    def start_thread(self):
        ...
        # Reset UI and state (Note: Automatically clears UI log on start)
        self.txt_log.config(state="normal")
        self.txt_log.delete(1.0, tk.END)
        self.txt_log.config(state="disabled")
    ...
    def perform_file_ops(self, path):
        try:
            ...
            if not self.dry_run_active:
                ...
                win_path = os.path.normpath(path) # Fixes forward slash Windows errors
